...
class ConfigController extends Controller
{
	...
	{
		$model=Config::model()->getModel("CONFIG-ID",array('configID'=>'1'));
		if (empty($model))
		$model = new Config();
		
		$image_name = $model->logo;
		if(isset($_POST['Config']))
		{
			$model->attributes=$_POST['Config'];
		$modelObject = CUploadedFile::getInstance($model, 'logo');
            ...
            {
                $image_name = md5($modelObject->name . time()) . '.' . substr($modelObject->type, 6);
               
                $modelObject->saveAs(BackendApplicationConfig::app()->params['folder_path']['applicationLogo'].$image_name);
